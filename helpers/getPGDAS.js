function getPGDAS(content, cnpj, razao) {
    ...
    if (posTitle !== -1) {
        ...
        newContent = newContent.replace(/Notificação de lançamento:\s+\d{17}/gmi, '')
        ...
        for (let index = 0; index < array.length; index++) {
            if (aux == 0 && !array[index].includes('PGDAS-D')) {
                continue
            }
            canReturn = true
            if (aux === 5) {
