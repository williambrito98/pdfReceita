function getDASByPendenciaParcelamento(content, cnpj, razao) { 
    ...
    if (posTitle !== -1) {
        ...
        newContent = newContent.replace(/(Pendência - Parcelamento \(PARCSN\/PARCMEI\))\s+_+/gmi, '$1 ')
        ...
        newContent = newContent.replace('Pendência - Parcelamento (PARCSN/PARCMEI)', '')
        newContent = newContent.replace('Débito com Exigibilidade Suspensa (SIEF)', '')
        newContent = newContent.replace('Parcelamento com Exigibilidade Suspensa (PARCSN/PARCMEI)', '')
        newContent = newContent.replace(/(Parcelas em atraso)\n\n(\d{1})/, '$1: $2')
